...
class PostsViewsTest(TestCase):
    ...
    def test_pages_uses_correct_template(self):
        ...
        templates_pages_names = {'posts/index.html': reverse('posts:index'),
                                 'posts/group_list.html': reverse('posts:group_list', kwargs={'slug': PostsViewsTest.group_one.slug}),
                                 'posts/profile.html': reverse('posts:profile', kwargs={'username': str(PostsViewsTest.user)}),
    ...
    def test_index_page_show_correct_context(self):
        ...
        data_for_test = {reverse('posts:index'):(mixer.blend(Post, author=PostsViewsTest.author, group = PostsViewsTest.group_two ),
                                                 Page),
                         }
        for path, (expected_value, feild_type) in data_for_test.items():
            response = PostsViewsTest.author_user.get(path)
            test_object = self.get_object_response_context(response, feild_type)
    ...
    def test_group_list_page_show_correct_context(self):
        ...
        expected_value= mixer.blend(Post, author=PostsViewsTest.author, group = PostsViewsTest.group_two )
    ...
    def test_profile_page_show_correct_context(self):
        ...
        expected_value= mixer.blend(Post, author=PostsViewsTest.author, group = PostsViewsTest.group_two )
        path = reverse('posts:profile',kwargs={'username':str(PostsViewsTest.author)})
        response = PostsViewsTest.author_user.get(path)
        paginator_page = self.get_object_response_context(response, Page)
        test_post = paginator_page.object_list[0] 
        self.assertEqual(test_post.text, expected_value.text)
    ...
    def test_post_detail_page_show_correct_context (self):
        ...
        expected_value= mixer.blend(Post, author=PostsViewsTest.author, group = PostsViewsTest.group_two )
        path = reverse('posts:post_detail', kwargs={'post_id':expected_value.id})
        response = PostsViewsTest.authorized_user.get(path)
    ...
    def test_post_create_show_correct_context (self):
        ...
        response = PostsViewsTest.authorized_user.get(path)
        test_value = PostsViewsTest.get_object_response_context(response,PostForm )
    ...
    def test_post_edit_page_show_correct_context(self):
        ...
        instance = mixer.blend(Post, author=PostsViewsTest.author, group = PostsViewsTest.group_two)
    ...
    def test_first_page_contains_ten_records(self):
        ...
        data_for_test = ( reverse('posts:index'),
                          reverse('posts:group_list', kwargs={'slug': PostsViewsTest.group_one.slug}),
                          reverse('posts:profile', kwargs={'username': str(PostsViewsTest.user)}))
