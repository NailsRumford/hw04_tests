...
class PostsURLTest(TestCase):
    ...
    def test_URL_for_all_users(self):
        ...
        data_for_test = {'/': 'posts/index.html',
                        ...
                        f'/profile/{PostsURLTest.user}/': 'posts/profile.html'}
    ...
    def test_URL_for_autor_user(self):
        ...
        data_for_test = {address:(adress_template, redirect_address)}
        """
        data_for_test = {f'/posts/{PostsURLTest.post.id}/edit/':('posts/create_post.html', f'/posts/{PostsURLTest.post.id}/')}
        for address, (template, redirect_address) in data_for_test.items():
            with self.subTest(test_address = address):
                response_author_user = PostsURLTest.author_client.get(address)
                response_authorized_user = PostsURLTest.authorized_client.get(address)
                response_guest_users = PostsURLTest.guest_client.get(address)
                self.assertEqual(response_author_user.status_code, HTTPStatus.OK)
                self.assertTemplateUsed(response_author_user, template)
                self.assertRedirects(response_authorized_user, redirect_address)
                self.assertRedirects(response_guest_users,('/auth/login/?next='+ address))
    
    def test_unexisting_page (self):
        data_for_test = {'/group/unexisting_page/','/profile/unexisting_page/'}
        for address in data_for_test:
            with self.subTest(test_address = address):
                response_goust_user = PostsURLTest.guest_client.get(address)
                response_authorized_user = PostsURLTest.author_client.get(address)
                self.assertEqual(response_goust_user.status_code,HTTPStatus.NOT_FOUND)
                self.assertEqual(response_authorized_user.status_code,HTTPStatus.NOT_FOUND)